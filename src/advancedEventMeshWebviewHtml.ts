...
export interface AdvancedEventMeshProviderTabs {
  readonly classicAvailable: boolean;
}

export function buildAdvancedEventMeshWebviewHtml(
  extensionUri: vscode.Uri,
  webview: vscode.Webview,
  appId: string,
  providerTabs: AdvancedEventMeshProviderTabs
): string {
  const prototypesUri = vscode.Uri.joinPath(extensionUri, 'docs', 'designs', 'prototypes');
  const scriptUri = webview.asWebviewUri(
    vscode.Uri.joinPath(prototypesUri, 'assets', 'advanced-events-webview.js')
  );
...
<head>
  ...
  <title>Advanced Event Mesh</title>
  <style nonce="${nonce}">
    @font-face {
      font-family: 'Outfit';
...
<body class="vscode-dark">
  ...
  <script nonce="${nonce}">
    window.advancedEventMeshAppId = ${JSON.stringify(appId)};
    window.advancedEventMeshProviderTabs = ${JSON.stringify(providerTabs)};
  </script>
